...
export default function AdminLoginCredentialsCard({ user, userType = 'student', onResetPassword }) {
  ...
  const isTempActive = Boolean(user.mustChangePassword);
    ...
    try {
      if (onResetPassword) {
        ...
        if (res && res.temporaryPassword) {
          ...
          addToast('New temporary password generated successfully!', 'success');
        } else {
  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow-premium border border-outline-variant/20 space-y-5 font-body">
      {/* Header */}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-on-surface-variant">
              User identity &amp; password reset controls (Authorized Admin Only)
            </p>
          </div>
        </div>
        ...
        <div className="bg-surface-container-lowest p-4 rounded-xl border border-outline-variant/15 space-y-2">
          ...
          <div className="flex items-center justify-between gap-2 bg-white p-2.5 rounded-lg border border-outline-variant/20">
            ...
            <button
              ...
              className="px-2.5 py-1 rounded bg-surface-container hover:bg-surface-container-high text-secondary text-[11px] font-bold transition-colors flex items-center gap-1 shrink-0 cursor-pointer"
              title="Copy Username"
            >
          ...
          <p className="text-[10px] text-on-surface-variant italic">
            The registered email address serves as the login username.
          </p>
        </div>
        ...
        <div className="bg-surface-container-lowest p-4 rounded-xl border border-outline-variant/15 space-y-2">
          ...
          <div className="flex items-center justify-between gap-2 bg-white p-2.5 rounded-lg border border-outline-variant/20">
            ...
            <button
              ...
              className="px-3 py-1 rounded bg-primary text-white text-[11px] font-bold hover:bg-primary-container transition-colors flex items-center gap-1 shrink-0 cursor-pointer shadow-sm"
            >
          ...
          <p className="text-[10px] text-on-surface-variant">
            Actual password is hidden and securely hashed. Admin can generate a new temporary password anytime.
          </p>
        </div>
      </div>
          ...
          <div className="bg-white rounded-3xl p-6 max-w-md w-full shadow-premium border border-outline-variant/20 space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-on-surface-variant mt-1 leading-relaxed">
                This will invalidate the user’s current password and generate a new temporary password for <strong className="text-secondary">{user.fullName || user.name}</strong> (<span className="font-mono text-primary">{username}</span>).
              </p>
            </div>
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              ...
              <button
                ...
                disabled={isResetting}
                className="px-4 py-2 rounded-xl text-xs font-bold text-white bg-rose-600 hover:bg-rose-700 transition-colors flex items-center gap-1.5 cursor-pointer shadow-sm"
              >
                {isResetting ? (
                  <>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-secondary/70 backdrop-blur-xs animate-fadeIn">
          <div className="bg-white rounded-3xl p-6 max-w-md w-full shadow-premium border border-emerald-200 space-y-4">
            ...
            <div className="bg-emerald-50/80 p-4 rounded-2xl border border-emerald-300/80 text-center space-y-2">
              <span className="text-[10px] font-bold text-emerald-900 tracking-wider block uppercase">Temporary Password</span>
              <div className="flex items-center justify-center gap-3">
                <span className="font-mono font-extrabold text-2xl text-emerald-950 tracking-wider select-all">{newTempPassword}</span>
                <button
                  type="button"
                  onClick={() => handleCopy(newTempPassword, 'Temporary Password')}
                  className="px-3 py-1.5 rounded-xl bg-emerald-700 hover:bg-emerald-800 text-white text-xs font-bold transition-all flex items-center gap-1 shadow-sm cursor-pointer"
                >
                  <span className="material-symbols-outlined text-[14px]">content_copy</span>
                  Copy Password
                </button>
            ...
            <p className="text-[11px] text-on-surface-variant bg-surface-container p-3 rounded-xl border border-outline-variant/15">
              This password will be required for the user's next login. Once closed, this plaintext password cannot be retrieved again.
            </p>
            ...
            <div className="pt-2 flex justify-end">
              <button
                ...
                className="px-5 py-2 rounded-xl bg-secondary text-white text-xs font-bold hover:bg-secondary/90 transition-colors cursor-pointer"
              >
                Done / Close
              </button>
            </div>
