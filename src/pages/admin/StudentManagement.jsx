  ...
  const filteredStudentLeaves = useMemo(() => {
    return studentLeaves.filter((l) => {
      const matchSearch =
        ...
        (l.admissionNo && l.admissionNo.toLowerCase().includes(leaveSearch.toLowerCase())) ||
        (l.className && l.className.toLowerCase().includes(leaveSearch.toLowerCase())) ||
        (l.reason && l.reason.toLowerCase().includes(leaveSearch.toLowerCase()));
  ...
  return (
    <div className="space-y-6 font-body">
      ...
      <div className="flex items-center gap-2 border-b border-outline-variant/20 px-2">
        <button
          onClick={() => {
            setSearchParams((prev) => { prev.set('tab', 'students'); return prev; });
          }}
          className={`px-5 py-3 font-headings font-bold text-xs border-b-2 transition-all flex items-center gap-2 cursor-pointer ${
            activeTab === 'students'
        ...
        <button
          onClick={() => {
            setSearchParams((prev) => { prev.set('tab', 'leaves'); return prev; });
          }}
          className={`px-5 py-3 font-headings font-bold text-xs border-b-2 transition-all flex items-center gap-2 cursor-pointer ${
            activeTab === 'leaves'
              ? 'border-primary text-primary bg-primary/5 rounded-t-xl'
      ...
      {activeTab === 'leaves' ? (
        <div className="space-y-6">
          {/* Multi-Filter & Search Toolbar for Student Leaves */}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 w-full md:w-auto">
                ...
                <div>
                  <select
                    ...
                    className="w-full px-3 py-2.5 rounded-xl border border-outline-variant/30 bg-surface-container-lowest text-xs font-bold text-secondary focus:outline-none"
                  >
                    ...
                    {filteredStudentLeaves.map((l) => (
                      <tr key={l._id || l.id} className="hover:bg-surface-container-lowest transition-colors">
                        <td className="py-3 px-4">
                          <div className="font-bold text-secondary text-xs">{l.studentName}</div>
                          <div className="font-mono text-[11px] text-primary">{l.admissionNo}</div>
                        </td>
                        <td className="py-3 px-4 whitespace-nowrap">
                          <span className="font-bold text-secondary block">Class {l.className}</span>
                          <span className="text-[11px] font-semibold text-on-surface-variant">🏢 {l.branch}</span>
                        </td>
                        <td className="py-3 px-4 font-bold text-purple-800 whitespace-nowrap">{l.leaveType}</td>
                        <td className="py-3 px-4 whitespace-nowrap">
                        ...
                        <td className="py-3 px-4 text-right whitespace-nowrap">
                          <button
                            ...
                            className="px-3 py-1.5 rounded-xl bg-primary text-white font-bold text-xs shadow-sm hover:bg-primary-container transition-all"
                          >
                            Review &amp; Manage
                          </button>
                        </td>
          ...
          <div className="space-y-4 font-body text-xs">
            ...
            <div className="pt-3 border-t border-outline-variant/15 flex justify-end gap-3">
              <button
                ...
                className="px-5 py-2.5 rounded-full border border-rose-300 bg-rose-50 text-rose-700 font-bold text-xs hover:bg-rose-100 transition-colors shadow-sm cursor-pointer"
              >
                Reject Application
              </button>
              <button
                ...
                className="px-6 py-2.5 rounded-full bg-emerald-600 text-white font-bold text-xs hover:bg-emerald-700 transition-all shadow-md cursor-pointer"
              >
                Approve Leave
              </button>
            </div>
