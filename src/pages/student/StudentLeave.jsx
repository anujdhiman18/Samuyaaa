...
export default function StudentLeave() {
  ...
  const fetchLeaves = async () => {
    ...
    try {
      ...
      const res = await studentService.getStudentLeaves();
      if (res && res.leaves) {
        const curId = String(currentUser?._id || currentUser?.id || '');
        const curAdmission = String(currentUser?.admissionNo || currentUser?.rollNumber || '').toLowerCase();

        // Filter to show ONLY the logged-in student's own leave applications
        const myLeaves = res.leaves.filter((l) => {
          const lId = String(l.studentId || '');
          const lAdm = String(l.admissionNo || '').toLowerCase();
          return (curId && lId === curId) || (curAdmission && lAdm === curAdmission);
        });

        // Show student's leaves if any, else return all demo student leaves
        setLeaves(myLeaves.length > 0 ? myLeaves : res.leaves);
      }
    } catch (err) {
      console.warn('Error fetching student leave applications:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleApply = async (e) => {
    ...
    try {
      ...
      const studentId = currentUser?._id || currentUser?.id || 's_' + Date.now();
      const admissionNo = currentUser?.admissionNo || currentUser?.rollNumber || 'ADM-2025-089';
      const studentName = currentUser?.fullName || currentUser?.name || 'Varun Sharma';
      ...
      if (res && res.success) {
        addToast('Student leave application submitted for admin approval!', 'success');
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      default:
        return (
          <span className="px-2.5 py-0.5 rounded-full text-[10px] font-medium bg-slate-100 text-slate-700 border border-slate-200 inline-flex items-center gap-1">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl shadow-premium border border-outline-variant/15">
        <div>
          ...
          <p className="font-body text-xs text-on-surface-variant mt-1">
            Apply for leave (Sick, Casual, Exam, Emergency) & track admin approval decisions.
          </p>
        </div>
        ...
        <form onSubmit={handleApply} className="space-y-4 font-body text-xs">
          <div>
            ...
            >
              ...
              <option value="Casual Leave">Casual Leave (Family Event / Emergency)</option>
              <option value="Exam Leave">Exam Leave (External Competitive Exam)</option>
          ...
          <div className="p-3 rounded-xl bg-primary/5 border border-primary/20 flex items-center justify-between">
            <span className="font-bold text-secondary text-xs">Total Leave Duration:</span>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="State clear reason for your leave request (e.g., Doctor advised 3 days rest for viral fever)..."
            />
          </div>

          <div>
            <label className="block text-xs font-bold text-secondary mb-1">Medical Certificate / Supporting Document URL (Optional)</label>
            <input
              type="url"
              value={supportingDocument}
              onChange={(e) => setSupportingDocument(e.target.value)}
              className="w-full px-3.5 py-2 rounded-xl border border-outline-variant/30 text-xs focus:outline-none focus:border-primary"
              placeholder="https://example.com/doctor-certificate.pdf"
            />
            <p className="text-[10px] text-on-surface-variant mt-1">Upload medical prescription or official document link if applicable.</p>
          </div>
